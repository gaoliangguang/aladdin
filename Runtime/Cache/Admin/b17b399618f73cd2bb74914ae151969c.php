...
<html lang="cn">
	...
	<div class="table-responsive">
        <div class="dataTables_wrapper">  
			<div class="col-sm-4">
                    <label>
                        <a href="<?php echo U('Freight/addFreight');?>" class="btn btn-white">新增运费模板</a>
                    </label>
                </div>
            <!-- 数据列表 -->
            <table class="table table-striped table-bordered table-hover dataTable">
				...
				<tbody>
					<?php if(!empty($_list)): foreach ($_list as $k => $v): ?>
					<tr>
						<td><?php echo $v['freightname'];?></td>
                        <td>￥<?php echo round(($v['firstfreight']/100),2);?></td>
                        <td>￥<?php echo round(($v['secfreight']/100),2);?></td>
                        <td><?php  switch ($v['freighttype']){ case 'NOT': echo '包邮'; break; case 'NOE': echo '包邮(指定区域除外)'; break; case 'BUY': echo '运费到付'; break; case 'NAT': echo '全国统一价'; break; case 'NAE': echo '全国统一价(指定区域除外)'; break; } ?></td>
                        <td><?php  switch ($v['fullstatus']){ case 'USE': echo ('启用'.'('.$v['fullsum'].')'); break; case 'FOR': echo '关闭'; break; } ?></td>
                        <td>
                            ...
                            <?php if($v['freighttype']=='NOE'||$v['freighttype']=='NAE'){ $url = U('areaList', array('id'=>$v['id'],'p'=>I('get.p', 1)), false); echo ("<a href='$url'>指定地区</a>"); } ?>
                        </td>
					</tr>
					<?php endforeach ?>
					<?php else: ?>
